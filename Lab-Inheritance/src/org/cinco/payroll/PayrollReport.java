			...
			while(s.hasNext()) {
				...
				if(!line.trim().isEmpty()) {
					...
					if(tokens[0].equals("E")) {
						...
						e = new Staff(title, firstName, lastName, id, hourlyPayRate, hoursWorked); //TODO: modify this
					} else if(tokens[0].equals("T")) {
						e = new Temporary(title, firstName, lastName, id, hourlyPayRate, hoursWorked); //TODO: modify this
					}
	...
	public static void main(String args[]) {
		...
		for(Employee e : payroll) {
			...
			sb.append(String.format("%-8s %-20s %-10s %-30s $%8.2f $%8.2f $%8.2f\n", 
					e.getID(), e.getlastName()+ "," + e.getFirstName(), e.getType(), e.getTitle(), e.getGrossPay(), e.getTaxes(), e.getNetPay())); //TODO: replace these
		}
